...
public class PersonResource {
    ...
    public List<Person> getAllPeople(
        ...
        if ("image-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where image is null");
            return StreamSupport.stream(personRepository.findAll().spliterator(), false)
                .filter(person -> person.getImage() == null)
                .toList();
        }

        if ("comment-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where comment is null");
            return StreamSupport.stream(personRepository.findAll().spliterator(), false)
                .filter(person -> person.getComment() == null)
                .toList();
        }

        if ("message-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where message is null");
            return StreamSupport.stream(personRepository.findAll().spliterator(), false)
                .filter(person -> person.getMessage() == null)
                .toList();
        }
