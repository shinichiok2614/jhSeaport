...
// slice
export const getPersonByUserId = createAsyncThunk(
  'person/fetch_person_by_userid',
  async (userId: number) => {
    const requestUrl = `${apiUrl}/person-by-user/${userId}`;
    return axios.get<IPerson>(requestUrl);
  },
  { serializeError: serializeAxiosError },
);
...
export const PersonSlice = createEntitySlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(getPersonByUserId.fulfilled, (state, action) => {
        state.loading = false;
        state.entity = action.payload.data;
      })
      ...
      .addMatcher(isPending(getEntities, getEntity, getPersonByUserId), state => {
        state.errorMessage = null;
        state.updateSuccess = false;
        state.loading = true;
      })
      .addMatcher(isRejected(getPersonByUserId), state => {
        state.loading = false;
        // Không cập nhật errorMessage để tránh thông báo lỗi
      })
